...
import 'fisioterapis_chat_tab.dart';
import 'fisioterapis_payment_history_screen.dart';
// Pastikan file ini sudah dibuat dengan nama yang sama persis di folder lib/screens/
...
class _FisioterapisHomeTabState extends State<FisioterapisHomeTab> {
            ...
            children: [
              _buildHeaderIconButton(
                ...
                onTap: () => Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (_) => const FisioterapisChatTab(),
                  ),
                ),
              ),
          ...
          _buildActionCard(
            ...
            onTap: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => const FisioterapisPaymentHistoryScreen(),
                ),
              );
            },
          ),
  ...
  void _showBookingRequestNames(BuildContext context) {
    final bookingRequests = BookingRequestRepository.requests
        .where((request) => request.status == BookingRequestStatus.waiting)
        .toList();

    if (bookingRequests.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Tidak ada permintaan booking saat ini')),
      );
      return;
    }
        ...
        children: [
          Row(
            children: [
              ...
              GestureDetector(
                onTap: () {
                  // REVISI: Hapus kata 'const' di depan FisioterapisTambahEdukasi()
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                        builder: (_) => FisioterapisTambahEdukasi()),
                  );
                },
                child: Row(
                  children: [
